...
| P1-C CodeCard production automation | Go/Flutter/Node security gates, Linux Chromium, extended PostgreSQL/MinIO gate and opt-in DeepSeek gate | **HEADLESS AUTOMATION PASS / LIVE QUALITY FAIL / WINDOWS DEVICE NOT RUN** | Automation remains green; real `deepseek-v4-pro` run `29466241445` at `9a3bd64` completed 0/20 after 60 calls, so live quality is explicitly FAIL pending stable-category diagnosis; see `docs/verification/p1c-codecard-production.md` |
