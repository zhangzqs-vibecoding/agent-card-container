...
| 4 | 远程环境仍可能使用内存 repository 和 `memory://` | **LOCAL HEADLESS PASS / REMOTE OPEN** | PostgreSQL/MinIO 持久化、重启、网络下载、成对备份恢复和同卡迭代均通过本地 Docker 门禁；固定远程服务器与 Windows 可达性尚未验收。 |
...
| 9 | CodeCard 正式 builder 和真实生成链路 | **HEADLESS AUTOMATION PASS / LIVE QUALITY PASS / DEVICE NOT RUN** | 受控 builder、固定评测合同、Linux Chromium 离线/RPC、签名发布及 PostgreSQL/MinIO 恢复已通过；收紧模型测试文件契约后，真实 run `29549396016` 为 19/20；Windows WebView2 仍未执行。 |
...
| P1-C CodeCard 生产链路 | **HEADLESS AUTOMATION PASS / LIVE QUALITY PASS / DEVICE NOT RUN** | 受控 builder、20 例固定质量合同、本地 JS SDK、Linux Chromium 离线/RPC、签名发布、真实 PostgreSQL/MinIO 恢复及 DeepSeek 19/20 质量门 | 在 Windows 参考设备完成离线 WebView2 矩阵 | Windows 11 x64 参考设备 |
...
- `docs/verification/p1a-cloud-card-iteration.md`：云端同卡迭代、签名基线、严格版本号及 PostgreSQL/MinIO 恢复证据。
